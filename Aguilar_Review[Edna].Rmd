...
* She has to clarify that the setwdd() in the creation of the files has to be change according to the final user or just removed. 
* The own visualization has some inconvenients and we have to include in the geom_smooth() the method 'loess'.
...
[1] 2782 986 Fang et all [1] 983 15 snp position

#both data sets are data frames #Types of files list

#class(fang_et_al) [1] "data.frame"
class(snp_position)
...
#Joining the data. the extracted data set( maize and teosinte) merged with the extracted_snp position
```{r}
...
#Extracting files for each chromosome. #Split the data into the 10 different chromosomes. I used a for loop #for this, i had to Set the directory where i wouldlike to save the extracted files. These files have the missing data represented by ?
...
ggplot(Genotype_joined, aes(x=count_amplicons, y=count_gene)) + geom_point() + geom_smooth(method = 'loess')# count amplicons vs gene
